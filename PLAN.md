...
> ⚠️ **【历史输入 · 已弃用】** 本文档立场已被 `PLAN_MERGED.md`（唯一现行来源）取代，仅保留作需求与原始背景记录。写作引用 `PLAN_MERGED.md`，勿以本文档结论为准。
> 📌 2026-08-31：项目已更名为 **mdpkg**（见 `PLAN_MERGED.md` §15）。本文档保留历史名称 MDE 以记录当时决策，不作为现行立场。
